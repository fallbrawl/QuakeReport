...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes() throws JSONException {
        ...
        JSONObject totalQuake = new JSONObject(SAMPLE_JSON_RESPONSE);
        JSONArray quakeArray = totalQuake.getJSONArray("features");

        for (int i = 0; i < quakeArray.length();i ++){
            JSONObject oneQuake = quakeArray.getJSONObject(i);
            JSONObject properties = oneQuake.getJSONObject("properties");
            Earthquake earthquake = new Earthquake(properties.getDouble("mag"),properties.getString("place"), properties.getLong("time"), properties.getString("url"));
